export class Magnetometer {
    constructor(scene, slot, serverComputer, options = {}) {
        ...
        this.detectionRadius = options.detectionRadius || 2.5; // ~2.5 meters, realistic for in-pavement sensors
    ...
    _buildMesh() {
        // Visual representation: A small LED stud embedded in the asphalt
    ...
    checkOccupancy(allCars) {
        ...
        for (const car of allCars) {
            // 1. Physical proximity check (simulates magnetic field disruption)
            const dx = car.root.position.x - this.slot.x;
            const dz = car.root.position.z - this.slot.z;
            const distance = Math.sqrt(dx * dx + dz * dz);
            
            // 2. Logical assignment check (fallback for cars perfectly centered or initializing)
        ...
        // Only update and notify server if state actually changed (saves compute/comm bandwidth)
        if (this.isOccupied !== detected) {
            this.isOccupied = detected;
            this._updateVisuals();
            
            // 📡 Report to Server Computer
